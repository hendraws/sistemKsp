      ...
      <td>
        <h4>Pengeluaran</h4><br>
                        ...
                        <?php 
                          ...
                          $q5       = mysqli_query($con,"select a.*,b.bu_nama from tbl_operasional a left join tbl_biayaumum b on a.bu_id=b.bu_id where a.operasional_tgl like '$bulan%' and b.bu_kategori='1' order by a.operasional_tgl asc");
                        while($h5   = mysqli_fetch_array($q5,MYSQLI_ASSOC)){
                         
                          $nominal_lain_arr[]      = $h5['nominal'];

                        }
                        $kasbon_pagi_total = array_sum($kasbon_pagi_arr);
                        $kembali_kasbon = $kasbon_pagi_total-$kasbon_pakai;
                        $biaya_lain = array_sum($nominal_lain_arr);
                       // echo "($kas_awal-$kasbon_pagi_total-$akomodasi-$operasional-$bon_panjere-$bon_privee-$biaya_lain)+$kembali_kasbon+$tunai_kas = ";
                        $total_kas_expedisi=($kas_awal-$kasbon_pagi_total-$akomodasi-$operasional-$bon_panjere-$bon_privee-$biaya_lain)+$kembali_kasbon+$tunai_kas;
                        ?>
      </td>
    </tr>
    <tr bgcolor="gray">
        ...
        <tr height="19" bgcolor="gray">
            ...
            <td align="center">
                Tab.
                        ...
                        <?php 
                        $no=1;
                        $q    = mysqli_query($con,"select a.pegawai_id,a.pegawai_nik,a.pegawai_nama,b.jabatan_nama,a.jabatan_id,a.pegawai_jk from tbl_pegawai a left join tbl_jabatan b on a.jabatan_id=b.jabatan_id where a.pegawai_id!='admin' order by a.jabatan_id asc");
        ...
        <?php
          ...
          while($h    = mysqli_fetch_array($q,MYSQLI_ASSOC)){
                ...
                <tr class="" bgcolor="<?php echo $warna;?>">
                          ...
                          <td align="center"><?php echo str_replace(",", ".", number_format($h['inven_transport']));?></td>
       ...
       <table cellpadding="7" style="width:100%">
                      ...
                      <thead>
                       ...
                       $q100=mysqli_query($con,"SELECT * from tbl_saldo_simpanan where saldo_bulan like '$bulan_lalu%' and resort_id='$resort_id'");
                          ...
                          while($h11=mysqli_fetch_array($q101)){
                              $minggu     = $h11['minggu'];
                              //senn kamis
                              //echo "senin-kamis";
                             
                              if($minggu=="1"){
                              $saldo_pinjaman1_lalu[]  = $h11['saldo_pinjaman'];
                              
                              }
                              //selasa - jum'at
                              if($minggu=="2"){
                              $saldo_pinjaman2_lalu[]  = $h11['saldo_pinjaman'];
                              
                              }
                           
                              // rabu  sabtu
                              if($minggu=="3"){
                              $saldo_pinjaman3_lalu[]  = $h11['saldo_pinjaman'];
