...
$qown 	= mysqli_query($con,"select * from tbl_owner limit 1");
		...
		<div class="container-login100" style="background-color: #DAD5D4;">
					...
					<div class="row">
						<div class="col-md-12">
							...
							<div class="wrap-input100 " data-validate = "Login sebagai">
								<input type="radio" class="radio" name="login_sebagai" value="1" checked="checked"> Admin
								<input type="radio" class="radio" name="login_sebagai" value="2"> PDL
								<input type="radio" class="radio" name="login_sebagai" value="3" id="kasir"> Kasir
								<input type="radio" class="radio" name="login_sebagai" value="4"> Juru Buku
							</div>
							...
							<div class="p-t-13 p-b-9 cabang" id="cabang" style="display: none;">
								<span class="txt1">
									CABANG
								</span>
							</div>
							<div class="wrap-input100 validate-input cabang" data-validate = "Password is required"  style="display: none;">
								<select name="cabang" class="input100" id="cabangSelect" disabled>
									<?php 
									$qunit = mysqli_query($con,"SELECT * from tbl_unit ");
									while($data_unit    = mysqli_fetch_array($qunit,MYSQLI_ASSOC)){
										$unit_id     = $data_unit['unit_id'];
										?>
										<option value="<?php echo $data_unit['unit_id']?>" ><?php echo $data_unit['unit_nama']?></option>
									<?php } ?>
								</select>
								<span class="focus-input100"></span>
							</div>
			...
			<script type="text/javascript">
				$('#username').keyup(function(){
					if($(this).val() == 'kasir' && $('#kasir').is(':checked')){
						$('.cabang').show();
						$('#cabangSelect').prop('disabled', false);
					}else{
						$('.cabang').hide();
							$('#cabangSelect').prop('disabled', true);
					}
				});

				$('.radio').click(function(){
					if($('#username').val() == 'kasir' && $('#kasir').is(':checked')){
						$('.cabang').show();
						$('#cabangSelect').prop('disabled', false);
					}else{
						$('.cabang').hide();
